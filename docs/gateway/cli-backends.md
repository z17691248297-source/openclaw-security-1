...
- `resumeArgs: ["exec","resume","{sessionId}","--json","--color","never","--sandbox","workspace-write","--skip-git-repo-check"]`
- `output: "jsonl"`
- `resumeOutput: "jsonl"`
- `modelArg: "--model"`
- `imageArg: "--image"`
...
- **Codex CLI trusted exec gating** uses Codex `command_execution` JSONL
  events when `tools.trustedIsolation` is enabled. OpenClaw still does not
  inject its native tool objects into CLI backends.
