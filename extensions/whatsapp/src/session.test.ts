...
const proxyAgentMocks = vi.hoisted(() => ({
  ctor: vi.fn(function MockHttpsProxyAgent(this: { proxyUrl: string }, proxyUrl: string) {
    this.proxyUrl = proxyUrl;
  }),
}));

vi.mock("https-proxy-agent", () => ({
  HttpsProxyAgent: proxyAgentMocks.ctor,
}));
...
describe("web session", () => {
  beforeEach(async () => {
    ...
    vi.clearAllMocks();
    proxyAgentMocks.ctor.mockClear();
  ...
  it("injects a proxy agent when HTTPS_PROXY is configured", async () => {
    vi.stubEnv("HTTPS_PROXY", "http://proxy.test:8080");

    await createWaSocket(false, false);

    expect(proxyAgentMocks.ctor).toHaveBeenCalledWith("http://proxy.test:8080");
    const makeWASocket = baileys.makeWASocket as ReturnType<typeof vi.fn>;
    const passed = makeWASocket.mock.calls[0]?.[0] as {
      agent?: { proxyUrl?: string };
      fetchAgent?: unknown;
    };
    expect(passed.agent?.proxyUrl).toBe("http://proxy.test:8080");
