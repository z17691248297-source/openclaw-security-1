...
function resolveWhatsAppProxyUrl(env: NodeJS.ProcessEnv = process.env): string | undefined {
...
function createWhatsAppProxyAgent(
  logger: ReturnType<typeof getChildLogger>,
  env: NodeJS.ProcessEnv = process.env,
): Agent | undefined {
  const proxyUrl = resolveWhatsAppProxyUrl(env);
  if (!proxyUrl) {
    return undefined;
  }
  try {
    ...
    logger.warn({ error: String(err), proxyUrl }, "invalid WhatsApp proxy configuration");
    return undefined;
  }
}
...
export async function createWaSocket(
  ...
  const proxyAgent = createWhatsAppProxyAgent(sessionLogger);
  // Baileys uses both undici fetch and a ws client. Bootstrapping the global
  // env-proxy dispatcher keeps the version probe aligned with the ws agent.
