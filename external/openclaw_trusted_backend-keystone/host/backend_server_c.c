...
#define DEFAULT_CA_BINARY "./keystone_example_openclaw_trusted_backend"
	...
	const struct exec_spec *spec)
{
	...
	if (binding->remote_platform[0] &&
	    strcasecmp(binding->remote_platform, "keystone") != 0)
		return false;
...
static int parse_args(int argc, char *argv[], struct server_config *config)
{
	...
	copy_text(config->upstream_tdx_public_key_file,
		  ...
		  getenv("KEYSTONE_TRUSTED_UPSTREAM_TDX_PUBLIC_KEY_FILE"));
	config->scope_token_ttl_ms =
		coerce_int(getenv("KEYSTONE_SCOPE_TOKEN_TTL_MS"),
			   DEFAULT_SCOPE_TOKEN_TTL_MS);
	config->confirmation_ttl_ms =
		coerce_int(getenv("KEYSTONE_CONFIRMATION_TTL_MS"),
	...
	if (strcmp(config->verify_mode, "hmac-sha256") == 0 &&
	    !config->hmac_key[0]) {
		fprintf(stderr,
			"verify mode hmac-sha256 requires --hmac-key or KEYSTONE_TRUSTED_HMAC_KEY\n");
		return 1;
	}
	if (strcmp(config->verify_mode, "ed25519") == 0 &&
	    !config->signing_private_key_file[0]) {
		fprintf(stderr,
			"verify mode ed25519 requires --signing-private-key-file or KEYSTONE_TRUSTED_SIGNING_PRIVATE_KEY_FILE\n");
		return 1;
	}
	if (strcmp(config->verify_mode, "ed25519") == 0 &&
...
int main(int argc, char *argv[])
{
	...
	printf("openclaw-keystone-backend listening on http://%s:%d (ca=%s, verify_mode=%s)\n",
