...
function spawnBuildCommand(command, args) {
...
function runTsdownBuild() {
  ...
  const direct = spawnBuildCommand("pnpm", pnpmArgs);
  if (!direct.error || direct.error.code !== "ENOENT") {
    return direct;
  }
  return spawnBuildCommand("corepack", ["pnpm", ...pnpmArgs]);
}

const result = runTsdownBuild();
...
if (result.error) {
  console.error(`Failed to start build toolchain: ${result.error.message}`);
  process.exit(1);
}
