...
export function buildApprovalPendingMessage(params: {
  ...
  lines.push("Reply with: allow-once|allow-always|deny");
  lines.push(`If needed, use: /approve ${params.approvalSlug} allow-once`);
  lines.push(
    `Other explicit options: /approve ${params.approvalSlug} allow-always | /approve ${params.approvalSlug} deny`,
  );
