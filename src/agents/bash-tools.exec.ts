...
import { enforceTrustedScopeForExec } from "../security/trusted-isolation/scope.js";
...
export function createExecTool(
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      const requestedWorkdir = params.workdir?.trim() || defaults?.cwd || process.cwd();
      enforceTrustedScopeForExec({
        toolName: "exec",
        args: params,
        command: params.command,
        workdir: requestedWorkdir,
        env: params.env,
        expectedSid: defaults?.sessionKey?.trim() || undefined,
        config: resolveTrustedIsolationConfig(defaults?.agentId),
      });
      ...
      const rawWorkdir = requestedWorkdir;
