...
import { createTrustedCodexCliStreamBridge } from "../../security/trusted-layer/codex-cli.js";
...
export async function executePreparedCliRun(
  ...
  try {
    return await enqueueCliRun(queueKey, async () => {
      ...
      let pendingTrustedAbort: Error | undefined;
      let managedRunCancel: (() => void) | undefined;
      const trustedBridge = createTrustedCodexCliStreamBridge({
      ...
      const managedRun = await supervisor.spawn({
        ...
        input: stdinPayload,
        onStdout: (chunk) => {
          trustedBridge.onStdout(chunk);
        },
      });
      managedRunCancel = () => managedRun.cancel("manual-cancel");
      if (pendingTrustedAbort) {
        managedRun.cancel("manual-cancel");
      }
      const result = await managedRun.wait();
      await trustedBridge.waitForSettled();
      const trustedAbortError = trustedBridge.getFailure() ?? pendingTrustedAbort;
      ...
      if (trustedAbortError) {
        throw trustedAbortError;
      }
