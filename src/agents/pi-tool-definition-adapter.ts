...
import { toTrustedIsolationError } from "../security/trusted-isolation/errors.js";
...
function describeToolExecutionError(err: unknown): {
  ...
  const trustedError = toTrustedIsolationError(err);
  if (err instanceof Error) {
    const message = err.message?.trim() ? err.message : String(err);
    return { message, stack: err.stack, errorCode: trustedError?.code };
  }
  return { message: String(err), errorCode: trustedError?.code };
}
