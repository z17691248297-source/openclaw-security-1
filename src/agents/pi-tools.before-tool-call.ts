...
import { enforceTrustedIsolationBeforeToolCall } from "../security/trusted-isolation/runtime.js";
...
export async function runBeforeToolCallHook(args: {
  ...
  const finalizeWithTrustedIsolation = async (inputParams: unknown): Promise<HookOutcome> => {
    const trustedOutcome = await enforceTrustedIsolationBeforeToolCall({
      toolName: args.toolName,
      inputParams,
      context: {
    ...
    if (trustedOutcome.blocked) {
      return { blocked: true, reason: trustedOutcome.reason };
    }
    return { blocked: false, params: trustedOutcome.params };
  };
  ...
  if (!hookRunner?.hasHooks("before_tool_call")) {
    return await finalizeWithTrustedIsolation(args.params);
  }
    ...
    if (hookResult?.requireApproval) {
      ...
      try {
        ...
        if (
          ...
          return await finalizeWithTrustedIsolation(
  ...
  return await finalizeWithTrustedIsolation(params);
}
