...
describe("pi-tools.policy", () => {
  ...
  it("keeps delete when write is allowlisted", () => {
    expect(isToolAllowedByPolicyName("delete", { allow: ["write"] })).toBe(true);
  });
  ...
  it("blocks delete when write is denylisted", () => {
    expect(isToolAllowedByPolicyName("delete", { deny: ["write"] })).toBe(false);
  });
});
