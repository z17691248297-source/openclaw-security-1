...
import { createHostWorkspaceDeleteTool, createSandboxedDeleteTool } from "./pi-tools.delete.js";
import {
...
export function createOpenClawCodingTools(options?: {
  ...
  const deleteTool = sandboxRoot
    ? allowWorkspaceWrites
      ? createSandboxedDeleteTool(
          { root: sandboxRoot, bridge: sandboxFsBridge! },
          { workspaceOnly },
        )
      : null
    : createHostWorkspaceDeleteTool(workspaceRoot, { workspaceOnly });
  const tools: AnyAgentTool[] = [
    ...base,
    ...(deleteTool
      ? [
          sandboxRoot && workspaceOnly
            ? wrapToolWorkspaceRootGuardWithOptions(deleteTool, sandboxRoot, {
                containerWorkdir: sandbox.containerWorkdir,
              })
            : deleteTool,
        ]
