...
describe("buildAgentSystemPrompt", () => {
  ...
  it("tells the agent not to execute /approve through exec", () => {
    ...
    expect(prompt).toContain(
      "When exec returns approval-pending, tell the user they can reply `allow-once`, `allow-always`, or `deny`;",
    );
    expect(prompt).toContain(
  ...
  it("guides subagent workflows to avoid polling loops", () => {
    ...
    expect(prompt).toContain(
      "For workspace file creation, edits, renames, or deletion, prefer write/edit/delete/apply_patch over exec commands like rm, mv, cp, mkdir, or touch when a file tool can do the job.",
    );
    expect(prompt).toContain(
      "When the user wants exactly one workspace file removed and the target is explicit or uniquely recoverable from the immediate context, call `delete` directly.",
    );
  });

  it("keeps delete in the ordered file-tool list", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      toolNames: ["exec", "apply_patch", "delete", "edit"],
    });

    const deleteIndex = prompt.indexOf("- delete: Delete a single workspace file;");
    const applyPatchIndex = prompt.indexOf(
      "- apply_patch: Apply multi-file patches, including file deletion",
    );

    expect(deleteIndex).toBeGreaterThanOrEqual(0);
    expect(applyPatchIndex).toBeGreaterThanOrEqual(0);
    expect(deleteIndex).toBeLessThan(applyPatchIndex);
  });
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/delete/apply_patch, file paths resolve against host workspace: /tmp/openclaw. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
