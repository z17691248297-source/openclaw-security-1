...
describe("/approve command", () => {
  ...
  it("accepts bare approval decisions for the current chat route", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true, id: "trusted:req-1" });
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    const params = buildParams("allow-once", cfg, {
      Provider: "whatsapp",
      Surface: "whatsapp",
      To: "+15555550123",
      AccountId: "work",
  ...
  it("does not hijack bare approval decisions when no current chat approval exists", async () => {
    callGatewayMock.mockRejectedValueOnce(new Error("unknown or expired approval id"));
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    const params = buildParams("deny", cfg, {
      Provider: "whatsapp",
      Surface: "whatsapp",
      To: "+15555550123",
    });
