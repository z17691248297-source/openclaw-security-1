...
export type TrustedIsolationToolConfig = {
  /** Enable operation-centric trusted isolation enforcement. */
  enabled?: boolean;
  /** Block trusted-required operations when the trusted backend is unavailable. */
  enforceFailClosed?: boolean;
  /** Trusted-plane API base URL (for authorize/complete RPCs). */
  backendBaseUrl?: string;
  /** Authorization endpoint path on the trusted backend. */
  authorizePath?: string;
  /** Completion/evidence endpoint path on the trusted backend. */
  completePath?: string;
  /** HTTP timeout for trusted backend requests. */
  requestTimeoutMs?: number;
  /** Trusted decision token TTL in milliseconds. */
  ttlMs?: number;
  verify?: {
    /** Signature verification mode for trusted scope tokens. */
    mode?: "none" | "hmac-sha256" | "ed25519";
    /** Shared HMAC key when mode=hmac-sha256. */
    hmacKey?: string;
    /** Trusted public key PEM when mode=ed25519. */
    publicKeyPem?: string;
    /** Require a scope token for trusted decisions. */
  ...
  levelToDecision?: {
    low?: "direct" | "notify" | "trusted" | "deny";
    medium?: "direct" | "notify" | "trusted" | "deny";
    high?: "direct" | "notify" | "trusted" | "deny";
    critical?: "direct" | "notify" | "trusted" | "deny";
  };
  /** Actions that always require trusted authorization. */
  forceTrustedActions?: Array<
    "read" | "modify" | "delete" | "exec" | "export" | "network" | "unknown"
  >;
  /** Tool names that always require trusted authorization. */
  forceTrustedTools?: string[];
  /** Object/path prefixes treated as sensitive and scored higher. */
  protectedPathPrefixes?: string[];
  /** Object/path prefixes that are always denied. */
  denyObjectPrefixes?: string[];
  /** Optional trusted evidence JSONL output path. */
  evidenceFile?: string;
};
...
export type AgentToolsConfig = {
  ...
  /** Trusted isolation policy overrides for this agent. */
  trustedIsolation?: TrustedIsolationToolConfig;
  sandbox?: {
    tools?: {
      allow?: string[];
...
export type ToolsConfig = {
  ...
  /** Runtime loop detection for repetitive/ stuck tool-call patterns. */
  loopDetection?: ToolLoopDetectionConfig;
  /** Global trusted isolation controls. */
  trustedIsolation?: TrustedIsolationToolConfig;
