...
describe("exec approval handlers", () => {
  ...
  type ExecApprovalResolveRouteArgs = Parameters<
    ExecApprovalHandlers["exec.approval.resolveRoute"]
  >[0];
  ...
  async function resolveExecApprovalRoute(params: {
    handlers: ExecApprovalHandlers;
    respond: ReturnType<typeof vi.fn>;
    context: { broadcast: (event: string, payload: unknown) => void };
    route: {
    ...
    return params.handlers["exec.approval.resolveRoute"]({
      params: params.route as ExecApprovalResolveRouteArgs["params"],
      respond: params.respond as unknown as ExecApprovalResolveRouteArgs["respond"],
      context: toExecApprovalResolveContext(params.context),
      client: null,
      req: { id: "req-3", type: "req", method: "exec.approval.resolveRoute" },
      isWebchatConnect: execApprovalNoop,
    });
  }
  ...
  it("resolves a unique pending approval by current chat route", async () => {
    const { handlers, broadcasts, respond, context } = createExecApprovalFixture();
    ...
    const resolveRespond = vi.fn();
    await resolveExecApprovalRoute({
      handlers,
      respond: resolveRespond,
      context,
      route: {
    ...
    expect(resolveRespond).toHaveBeenCalledWith(true, { ok: true, id }, undefined);
    expect(respond).toHaveBeenCalledWith(
      true,
      expect.objectContaining({ id, decision: "allow-once" }),
      undefined,
